...
**Permission-subtraction pitfall** — `chrome.permissions.remove` subtracts everything the removed pattern *contains* from the active permission set. Removing `https://*/*` therefore also strips the **required** API `host_permissions` from the manifest (all https), and every AI fetch starts failing CORS ("No internet connection" in the tooltip). Two defenses in the code: (1) `onSave` in `options.js` re-requests `chrome.runtime.getManifest().host_permissions` inside the same click gesture on **every** save — silent, since those origins stay in Chromium's *granted* set — which both pairs with the untoggle removal and self-heals already-affected profiles; (2) per-domain removals (`removeFloatingDomain` in options, the context-menu toggle in background) skip `permissions.remove` when the host is covered by a manifest pattern. Never remove a broad origin pattern without re-requesting the manifest hosts in the same gesture.
