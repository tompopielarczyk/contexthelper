...
/**
 * Request permission for all given webhook URLs in a single prompt.
 * Must be called synchronously within a user gesture (before any await).
 * Resolves true when nothing needs granting.
 */
export async function requestWebhookPermissions(urls) {
  const origins = [...new Set(urls.map(originPatternForUrl).filter(Boolean))];
