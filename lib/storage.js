import { validateWebhookUrl, WebhookError } from './webhook.js';
...
export async function saveSettings(settings) {
  ...
  if (actions) {
    ...
    for (const action of actions) {
      ...
      if (action.webhookId && !webhookIds.has(action.webhookId)) {
        throw new Error(`Action "${action.name}" references unknown webhook`);
      }
    }
  }
  ...
  if (actions) syncPayload.actions = actions.map((a, index) => ({
    ...a,
    id: a.id || `legacy-action-${index}`,
    displayMode: a.displayMode || 'auto',
    webhookId: a.webhookId || '',
